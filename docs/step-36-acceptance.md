...
顯示標題另須包含「財務報告」及可辨識的季度、上半年度或年度字樣。
季度字樣優先於「年度」：例如「年度第二季」只提示 Q2；「上半年度」也提示 Q2，
不提示 FY。沒有季度或半年度字樣時，「年度財務報告」及「2025年個別財務報告」
這類年份加財報標題才提示 FY。「下半年度」不推斷為 FY。
解析器保留原顯示標題、完整 detail URL、清單 URL 與原始 hash；期別和
「合併／個體」只作**提示**，其中來源的「個別」歸一為「個體」。
未出現範圍字樣時，提示為空，不猜測。像「董事會預計召開日期」
