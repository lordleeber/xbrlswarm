...
Enum 中的宣告順序與 ROADMAP 一致；Step-28 固定 state transition 順序，Step-33 已開放 MOPS 語意耗盡後的轉移；Goodinfo 目前 pending，故轉到 Yahoo，其他跨來源派發仍須通過後續來源 gate，詳見 `docs/engine-fallback.md`。Step-27 的基礎設施失敗仍留在同一 engine。
