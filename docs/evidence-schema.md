...
| `filing_kind` | TEXT | 否 | 申報版本種類；正式值域由 Step-14 定義 |
...
Source locator、title、subject 與 raw payload hash 也只在實際可得時保存，不用假值填補。`filing_kind` 在現有 MOPS fixtures 仍是 `not_verified`，因此必須允許缺值。
...
- Step-10 不定義 filing kind、event precision 或 verification state 的正式 enum。
