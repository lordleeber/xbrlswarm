...
Step-46 決策（2026-09-26，驗收見 `docs/step-46-acceptance.md`）：

- 公司代號取自 `公司名稱：X (代號)` 行；四欄位缺一即 `not_mops_form`。
- 「優先」的實作方式：非 MOPS 形式的文章一律不接受。所有 MOPS 形式、身分相符
  的鏡像依候選順序保留。
- 期間只支援曆年制；起日為 1 月 1 日（累計）或該季起日。
- 沒有鏡像被接受時，任何一篇可重試的審查都讓 task 維持可重試；全部都有最終判定
  才是 `rejected`。404／410 視為該候選的最終判定。
- 實測：2019 年舊版重大訊息格式沒有報導期間，會被拒；頎邦 2024 Q3 從查詢到
  接受全程通過。
